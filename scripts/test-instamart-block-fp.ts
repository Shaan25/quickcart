...
async function main() {
  ...
  // Block the fingerprinting script AND mock fingerprint endpoint
  await context.route("**browserFingerPrint**", (route) => {
    console.log("🚫 Blocked fingerprint script:", route.request().url().slice(0, 80));
    route.fulfill({ status: 200, body: "// blocked" });
  });
  await context.route("**/fingerprint**", (route) => {
  ...
  page.on("response", async (res) => {
    ...
    try {
      ...
      console.log(`  body length: ${text.length}${text.length > 5 ? " ✅" : " ❌"}\n`);
      if (text.length > 5 && (url.includes("suggest-items") || url.includes("search/mart"))) {
