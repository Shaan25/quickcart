...
async function main() {
  ...
  page.on("response", async (res) => {
    ...
    try {
      ...
      if (text.length > 10 && (url.includes("suggest-items") || url.includes("search/mart"))) {
        fs.writeFileSync(`.instamart-fresh-${url.includes("mart") ? "mart" : "suggest"}.json`, text);
        console.log("  💾 SAVED!\n");
      }
    } catch {}
  });
  ...
  // Close the location modal overlay if present
  ...
  // Click the search button using force:true to bypass overlay
  ...
  await page.screenshot({ path: "instamart-fresh3.png" });
  await browser.close();
}
