...
async function main() {
  ...
  page.on("response", async (res) => {
    const url = res.url();
    if (!url.includes("instamart") || url.includes("media-assets") || url.includes("nr-data")) return;
    const ct = res.headers()["content-type"] ?? "";
    if (!ct.includes("json")) return;
    try {
      const text = await res.text();
      if (text.length < 1000) return;
      console.log(`[${res.status()}] ${url.slice(0, 100)} (${text.length} bytes)`);
      if (!url.includes("home/v2")) return;
      fs.writeFileSync(".instamart-home-full.json", text);
